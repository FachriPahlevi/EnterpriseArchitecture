...
class PortofoliosController extends VoyagerBaseController
{
    ...
    // Mengambil semua fase beserta artefak-nya
    $phasesWithArtefacts = Phase::with('artefact')->get();
    
    // Mendapatkan ID artefak yang dicentang untuk portofolio tertentu
    $checkedArtefacts = $portofolio->Component->pluck('artefact')->toArray();

    // Mendapatkan semua data artefak
    $artefacts = Artefact::all();

        
    

        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted',  'portofolio','checkedArtefacts','artefacts', 'phasesWithArtefacts'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $editing = false; // Inisialisasi variabel $editing

        if ($dataType && $dataType->editRows) {
            $editing = true; // Anda sedang dalam mode edit
        }
        
        $allArtefact = Artefact::all();
        $phasesWithArtefacts = Phase::with('Artefact')->get();

         // Mendapatkan data portofolio berdasarkan ID
    $portofolio = Portofolio::findOrFail($id);

    // Mendapatkan daftar artefak yang sudah dicentang untuk portofolio ini
    //dd($portofolio->components);
    $checkedArtefacts = $portofolio->Component->pluck('artefact')->toArray();


    // Mendapatkan informasi fase dan artefak
    $phasesWithArtefacts = Phase::with('artefact')->get();

        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'allArtefact', 'phasesWithArtefacts', 'checkedArtefacts', 'editing'));
    }
...
if ($portofolio) {
    $portofolio->update([
        // ... atribut portofolio lainnya yang ingin diperbarui
    ]);

    // Ambil artefak yang dipilih dari request
    $selectedArtefacts = $request->input('artefact', []);

    $portofolio->Component()->each(function ($Component) use ($selectedArtefacts) {
        if (!in_array($Component->artefact, $selectedArtefacts)) {
            // Komponen terkait tidak dipilih, hapus dari relasi
            $Component->delete();
        }
    });

    // Tambahkan komponen baru untuk artefak yang dipilih
    foreach ($selectedArtefacts as $artefactId) {
        $existingComponent = $portofolio->Component->where('artefact', $artefactId)->first();
        if (!$existingComponent) {
            $artefact = Artefact::find($artefactId);
            $phaseId = $artefact->phase;
            $typeIds = array_map('intval', $artefact->type->pluck('id')->toArray()); // Menambahkan ini
            foreach ($typeIds as $typeId) {
            $portofolio->Component()->create([
                'artefact' => $artefactId,
                'phase' => intval($phaseId),
                'type' => $typeId, // Menambahkan ini
                // ... atribut lainnya
            ]);
        }
        }
    ...
    public function create(Request $request)
    {
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'allArtefact', 'phasesWithArtefacts'));
    }
    ...
    {//dd($request->all());
        ...
        if (!$request->has('_tagging')) {
            ...
            if ($request->has('artefact')) {
                foreach ($request->artefact as $artefactId) {
                    $artefact = Artefact::find($artefactId);
                    // Pastikan Artefact ada
                    
                    if ($artefact) {
                        // Akses atribut 'phase' dari objek Artefact
                        $phase = $artefact->phase;
                        //$typeIds = array_map('intval', $artefact->type->pluck('id')->toArray());
                        //dd($typeIds);
                        $typeIds = array_map('intval', $artefact->type->pluck('id')->toArray());
                        foreach ($typeIds as $typeId) {
                    Component::create([
                        'portofolio_id' => $portofolioId, // Gunakan field yang sesuai
                        'artefact' => intval($artefactId),
                        'phase' => intval($phase),
                        'type' => $typeId, // Menggunakan implode untuk menggabungkan ID tipe 
                        // ... other columns
                    ...
                    // Dapatkan fase-fase yang dimiliki oleh artefact dan hubungkan dengan komponen
                    //$phases = $artefact->phases; // Asumsi ada relasi 'phases' di model Artefact
                    //$Component->Phases()->sync($phases->pluck('id')); //kalau error aktifkan ini
                    
                }  else {
            // Handle ketika Artefact tidak ditemukan
        }  
