...
class VoyagerController extends Controller
{
    public function index()
    {
        ...
        $userData = Portofolio::select(\DB::raw("status, COUNT(*) AS count"))
        ...
        $totalOn = 0; // Inisialisasi jumlah portofolio dengan status "on"
        $totalOff = 0; // Inisialisasi jumlah portofolio dengan status "off"
        ...
        foreach ($userData as $userData) {
            ...
            if ($userData->status === 'on') {
                $totalOn += $userData->count;
            } elseif ($userData->status === 'off') {
                $totalOff += $userData->count;
            }
        

           
        }
        $totalPortofolio = Portofolio::count(); // Menghitung jumlah portofolio
        $totalClient = Client::count(); // Menghitung jumlah portofolio

       // return Voyager::view('voyager::index', ['chartData' => $chartData]);
        return Voyager::view('voyager::index', ['chartData' => $chartData, 'totalPortofolio' => $totalPortofolio, 'totalClient' => $totalClient,'totalOn' => $totalOn, 'totalOff' => $totalOff,]);
